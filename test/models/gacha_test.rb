...
class GachaTest < ActiveSupport::TestCase
  ...
  test 'gacha returns same result when same items and seed given' do
    gacha1 = Gacha.create!(items: @items, seed: @seed)
    gacha2 = Gacha.create!(items: @items, seed: @seed)
    assert_equal gacha1.gacha, gacha2.gacha
  end

  test 'gacha returns not same result when same items and not same seed given' do
    gacha1 = Gacha.create!(items: @items, seed: @seed)
    gacha2 = Gacha.create!(items: @items, seed: Random.new_seed)
    assert_not_equal gacha1.gacha, gacha2.gacha
  end
